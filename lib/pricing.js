...
async function airtableGet(table, params = {}) {
  ...
  if (!res.ok) throw new Error(`Airtable GET ${table} failed: ${res.status}`);
  return res.json();
}
...
export async function selectBestSourceur(modele, finition, boite, couleur) {
  ...
  const data = await airtableGet('PRIX_SOURCEURS', {
    ...
    fields: JSON.stringify([
      'sourceur_id', 'prix_vehicule_usd', 'cif_shipping_sourceur_usd',
      'cif_shipping_calcule_usd', 'couleur', 'stock_disponible', 'pays_sourceur',
      'date_mise_a_jour',
    ]),
  });
  ...
  if (results.length === 0) {
    ...
    const allData = await airtableGet('PRIX_SOURCEURS', {
      filterByFormula: `AND({modele}="${modele}",{finition}="${finition}",{boite_vitesse}="${boite}",{stock_disponible}>0,{actif}=TRUE())`,
      fields: JSON.stringify(['couleur']),
    });
...
export async function confirmAvailability(sourceur_id, reference_dossier) {
  const data = await airtableGet('SOURCEURS', {
    ...
    maxRecords: 1,
    fields: JSON.stringify(['telephone', 'nom']),
  });
...
export async function selectFallbackSourceurs(modele, finition, boite, budget_usd, exclude_sourceur_id) {
  const data = await airtableGet('PRIX_SOURCEURS', {
    ...
    sort: JSON.stringify([{ field: 'prix_vehicule_usd', direction: 'asc' }]),
    fields: JSON.stringify(['sourceur_id', 'prix_vehicule_usd', 'cif_shipping_sourceur_usd', 'cif_shipping_calcule_usd', 'telephone_sourceur']),
  });
