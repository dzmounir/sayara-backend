...
const T = {
  PRIX_SOURCEURS:       process.env.AIRTABLE_PRIX_SOURCEURS_TABLE       || 'tbllk5ZWO6FYlAj3p',
...
import { checkAndNotifyLowStock } from './stock_alerts.js';
...
// ─── decrementStock — called ONLY when notaire confirms depot ─────────────────
export async function decrementStock(sourceur_id, marque, modele, annee, finition, boite, couleur) {
  const parts = [`{actif}=TRUE()`, `{stock_disponible}>0`];
  if (sourceur_id) parts.push(`{sourceur_id}="${sourceur_id}"`);
  if (marque)      parts.push(`{marque}="${marque}"`);
  if (modele)      parts.push(`{modele}="${modele}"`);
  if (annee)       parts.push(`{annee}=${annee}`);
  if (finition)    parts.push(`{finition}="${finition}"`);
  if (boite)       parts.push(`{boite_vitesse}="${boite}"`);
  if (couleur)     parts.push(`{couleur}="${couleur}"`);

  const data = await atGet(T.PRIX_SOURCEURS, {
    filterByFormula: `AND(${parts.join(',')})`,
    maxRecords: 1,
    'fields[]': ['stock_disponible', 'alerte_stock_envoyee', 'sourceur_id', 'marque', 'modele', 'finition', 'couleur', 'nom_entreprise'],
  });

  if (!data.records?.length) return { updated: false };

  const rec       = data.records[0];
  const f         = rec.fields;
  const newStock  = Math.max(0, (Number(f.stock_disponible) || 0) - 1);
  const updates   = { stock_disponible: newStock, date_mise_a_jour: new Date().toISOString() };
  if (newStock === 0) updates.actif = false;
  if (newStock >  2)  updates.alerte_stock_envoyee = false; // reset flag if stock recovered

  await atPatch(T.PRIX_SOURCEURS, rec.id, updates);
  await recalculateBestPrices();

  if (newStock <= 2 && newStock > 0 && !f.alerte_stock_envoyee) {
    checkAndNotifyLowStock(rec.id, newStock, {
      sourceur_id: f.sourceur_id,
      nom_sourceur: f.nom_entreprise || f.sourceur_id,
      marque: f.marque, modele: f.modele, finition: f.finition, couleur: f.couleur,
    }).catch(() => {});
  }
