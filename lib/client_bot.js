...
async function atGet(table, params = {}) {
  ...
  for (const [k, v] of Object.entries(params)) {
    if (k === 'fields' && typeof v === 'string' && v.startsWith('[')) {
      try { JSON.parse(v).forEach(f => url.searchParams.append('fields[]', f)); continue; } catch { /**/ }
    }
    if (k === 'sort' && typeof v === 'string' && v.startsWith('[')) {
      try {
        JSON.parse(v).forEach((s, i) => {
  ...
  if (!res.ok) throw new Error(`AT GET ${table}: ${res.status} ${await res.text().catch(() => '')}`);
  return res.json();
}
...
async function doVehiculeConfirm(from, choiceId, session) {
  ...
  console.log('[doVehiculeConfirm] criteria:', { marque: h.marque, modele: h.modele, annee: h.annee, confiance: h.confiance });

  try {
    const { available, finitions } = await getFinitionsDisponibles(h.marque, h.modele, h.annee);
    console.log('[doVehiculeConfirm] result:', { available, finitions });
    ...
    const isAirtable = err.message.includes('Airtable') || err.message.includes('failed:');
    console.error('[doVehiculeConfirm] erreur:', err.message, { marque: h.marque, modele: h.modele, annee: h.annee, isAirtable });
    if (isAirtable) {
      sendText(COURTIER, `🔴 ERREUR CATALOGUE\nClient: ${from}\nVéhicule: ${h.marque} ${h.modele} ${h.annee}\nErreur: ${err.message}`).catch(() => {});
      save(from, 'MENU');
      return sendText(from, '🔄 Catalogue momentanément indisponible.\nNotre équipe est alertée et vous répond dans quelques minutes. 📞');
    }
    save(from, 'VEHICULE_INPUT');
    return sendText(from, '🤔 Je n\'ai pas bien compris. Réessayez en précisant :\n\n▸ Ex: _duster 2025 automatique_\n\nOu tapez *menu* pour revenir.');
  }
}
