...
import { selectBestSourceur, calculateDevis } from './pricing.js';
...
export async function handleClientMessage(payload) {
  ...
  try {
    switch (session.step) {
      ...
      case 'VARIANTE_SELECT':   return doVarianteSelect(from, choiceId, listTitle, session);
...
async function sendMenu(from) {
  ...
  return sendButtons(from,
    ...
    [
      ...
      { id: 'MENU_SUIVI',  title: '📍 Suivre mon dossier' },
      { id: 'MENU_AGENT',  title: '👤 Parler à un conseiller' },
    ]
  );
}
...
async function doVehiculeConfirm(from, choiceId, session) {
  ...
  let records = [];
  try {
    const data = await atGet(TBL_CAT, {
      filterByFormula: `AND(LOWER({modele})=LOWER("${h.modele}"),{stock_disponible}>0)`,
      sort: JSON.stringify([{ field: 'prix_devis_da', direction: 'asc' }]),
      maxRecords: 8,
      fields: JSON.stringify(['modele', 'marque', 'finition', 'boite_vitesse', 'couleur', 'prix_devis_da', 'annee']),
    });
  ...
  if (!records.length) {
    ...
    await sendText(COURTIER, `📊 HORS STOCK\nNuméro: ${from}\nVéhicule: ${h.marque} ${h.modele} ${h.annee}\nTexte: "${session.data.vehicule_texte}"`);
    return;
  }

  const variantes = records.map(r => ({ id: r.id, ...r.fields }));
  save(from, 'VARIANTE_SELECT', { variantes, marque: h.marque, modele: h.modele, annee: h.annee });

  const rows = variantes.map(v => ({
    id:          `VAR_${v.id}`,
    title:       `${v.finition || v.modele} ${v.annee || ''}`.trim().slice(0, 24),
    description: `${v.boite_vitesse || ''} · ${v.couleur || ''} · ${fmt(v.prix_devis_da)}`.slice(0, 72),
  }));

  return sendList(from,
    'Quelle variante vous intéresse ? 👇\n_Prix indicatifs — tous inclus sauf douane._',
    '📋 Voir les variantes',
    [{ title: 'Variantes disponibles', rows }]
  );
}

// ─── PHASE 5A — VARIANTE ─────────────────────────────────────────────────────
async function doVarianteSelect(from, choiceId, listTitle, session) {
  const varId   = choiceId.replace('VAR_', '');
  const variante = session.data.variantes?.find(v => v.id === varId) || {};

  save(from, 'COULEUR_SELECT', {
    variante_id: varId, variante_label: listTitle, variante,
    finition: variante.finition || '', annee: variante.annee || session.data.annee,
  });

  const couleurStock = variante.couleur;
  const base = ['Blanc Glacier', 'Gris Highland', 'Noir Étoile', 'Marron / Beige'];
  const options = couleurStock ? [couleurStock, ...base.filter(c => c !== couleurStock)] : base;
  const rows = [...new Set(options)].slice(0, 9).concat(['Peu importe']).map((c, i) => ({
    id:          `CLR_${i}`,
    title:       c.slice(0, 24),
    description: c === couleurStock ? '✅ En stock confirmé' : 'Disponible',
  }));

  return sendList(from, 'Quelle couleur préférez-vous ? 🎨', '🎨 Choisir une couleur', [{ title: 'Couleurs', rows }]);
}

// ─── PHASE 5B — COULEUR ───────────────────────────────────────────────────────
async function doCouleurSelect(from, listTitle, session) {
  save(from, 'BOITE_SELECT', { couleur: listTitle });
  return sendButtons(from, 'Boîte de vitesses ? ⚙️', [
...
async function doBoiteSelect(from, choiceId, session) {
  const boite = choiceId === 'BOITE_AUTO' ? 'Automatique' : choiceId === 'BOITE_MANU' ? 'Manuelle' : 'Peu importe';
  save(from, 'FORMULE_SELECT', { boite });
...
async function doEmailInput(from, text, session) {
  ...
  try {
    const result = await selectBestSourceur(
      d.modele,
      d.variante?.finition || d.finition || null,
      d.boite === 'Peu importe' ? null : d.boite,
      d.couleur === 'Peu importe' ? null : d.couleur,
    );
...
async function doDevisConfirm(from, choiceId, session) {
  ...
  return sendButtons(from, 'Acceptez-vous les conditions du contrat ?', [
    { id: 'SIGN_OK',   title: '✍️ J\'accepte — Signer' },
...
async function checkDepotComplet(from) {
  ...
  if (d.dossier_id) {
    atPatch(TBL_DOS, d.dossier_id, { statut: 'DEPOT_CONFIRME', timestamp_confirmation_depot: new Date().toISOString() }).catch(() => {});
  }
...
async function handleNotaireReply(notairePhone, buttonId) {
  if (buttonId.startsWith('NOTAIRE_OK_')) {
    ...
    try {
      atPatch(TBL_DOS, dossier_id, { depot_recu_notaire: true }).catch(() => {});
      const rec = await atFetch(TBL_DOS, dossier_id);
      const clientPhone = rec.fields?.telephone;
      if (clientPhone) {
