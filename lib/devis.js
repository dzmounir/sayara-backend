...
export async function generateDevis(telephone, dossier_id, formule, remise_da = 0) {
  // 1. Fetch dossier
  ...
  const sourceurResult = await selectBestSourceur(
    f.modele, f.reference_variante, f.boite_vitesse, f.couleur_1 ?? 'PEU_IMPORTE'
  );
  ...
  // 4b. Apply broker discount
  const remise = Math.max(0, Math.round(remise_da ?? 0));
  if (remise > 0) {
    devis.total_livre_au_port = Math.max(0, devis.total_livre_au_port - remise);
    devis.total_dedouane      = Math.max(0, (devis.total_dedouane ?? 0) - remise);
    devis.remise_da = remise;
  }
  ...
  const carboneRes = await fetch(`https://api.carbone.io/render/${CARBONE_TEMPLATE_DEVIS}`, {
    ...
    body: JSON.stringify({
      data: {
        ...
        variante:               dossierFields.reference_variante,
        couleur:                dossierFields.couleur_1,
        boite_vitesse:          dossierFields.boite_vitesse,
        formule:        formule === 'FORMULE_DEDOUANE' ? 'Dédouané complet' : 'Livré au port',
        ...
        remise_da:              remise > 0 ? fmt(remise) : null,
...
function buildDevisMessage(f, devis, formule) {
  ...
  msg += `📅 Valable 48h\n\n`;
  msg += `*Véhicule :* ${f.marque ?? 'Dacia'} ${f.modele ?? 'Duster'} — ${f.reference_variante}\n`;
  msg += `*Couleur :* ${f.couleur_1 ?? 'À définir'} | *Boîte :* ${f.boite_vitesse}\n\n`;
  msg += `━━━━━━━━━━━━━━━━━━━\n`;
  msg += `💰 *Prix véhicule* : ${fmt(devis.prix_vehicule_affiche)} DA\n`;
  msg += `   _(${devis.note_prix})_\n`;
  msg += `📋 *Frais notaire* : ${fmt(devis.frais_notaire)} DA\n`;
  msg += `━━━━━━━━━━━━━━━━━━━\n`;
  if (devis.remise_da > 0) {
    msg += `🎁 *Remise accordée* : -${fmt(devis.remise_da)} DA\n`;
  }
